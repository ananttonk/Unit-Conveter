...
class CentimetreToKilometre : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.toolBarName.text = "Centimetre To Kilometre"
        binding.tvName.text = "Centimetre To Kilometre"
        binding.toolbarBackButton.setOnClickListener { onBackPressed() }
        binding.convertButton.setOnClickListener {
            if (binding.etFiled.text.toString().isNotEmpty()) {
                binding.tvResult.text = "Result: " + (binding.etFiled.text.toString()
                    .toFloat() / 100000.0).toString() + " (Kilometre)"
            } else {
