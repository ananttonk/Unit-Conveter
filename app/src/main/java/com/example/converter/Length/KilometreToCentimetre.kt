...
class KilometreToCentimetre : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.toolBarName.text = "Kilometre To Centimetre"
        binding.tvName.text = "Kilometre To Centimetre"
        binding.toolbarBackButton.setOnClickListener { onBackPressed() }
        binding.convertButton.setOnClickListener {
            if (binding.etFiled.text.toString().isNotEmpty()) {
                binding.tvResult.text = "Result: " + (binding.etFiled.text.toString()
                    .toFloat() * 100000).toString() + " (Centimetre)"
            } else {
